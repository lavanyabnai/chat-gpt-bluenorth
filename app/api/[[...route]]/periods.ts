...
const app = new Hono()
  ...
  .delete(
    ...
    async (c) => {
      ...
      const [data] = await db.delete(periods).where(eq(periods.id, parseInt(id))).returning();
...
.patch(
  ...
  async (c) => {
    ...
    const [data] = await db.update(periods).set(values).where(eq(periods.id, parseInt(id))).returning();
